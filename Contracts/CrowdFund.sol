...
*/
  ...
  function createCampaign(
    ...
    require(
      _fundingPeriodInDays >= 1 && _fundingPeriodInDays <= 365,
      "Funding Period should be between 1 -365 days"
    );
    ...
    Campaign memory aCampaign = Campaign(
      ...
      _goalAmount,
      0,
      now + (_fundingPeriodInDays * 1 days),
      false,
      true,
      true
    );
  ...
  function fundCampaign(uint256 _campaignID) public payable {
    ...
    campaigns[_campaignID].contributions[msg.sender] =
      (campaigns[_campaignID].contributions[msg.sender]) +
      msg.value;
    campaigns[_campaignID].totalAmountFunded =
      campaigns[_campaignID].totalAmountFunded +
      msg.value;
    ...
    if (
      campaigns[_campaignID].totalAmountFunded >=
      campaigns[_campaignID].goalAmount
    ) {
    ...
    onlyCampaignOwner(_campaignID)
  {
    require(campaigns[_campaignID].isExists, "Campaign does not exists");
    require(
      campaigns[_campaignID].goalAchieved,
      "Cant withdraw. Goal amount not reached"
    );
  ...
  function claimRefund(uint256 _campaignID) public {
    ...
    require(
      !(campaigns[_campaignID].isCampaignOpen) &&
        !(campaigns[_campaignID].goalAchieved),
      "Reclaim conditions not met"
    );
